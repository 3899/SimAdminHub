...
单独安装的 Host Agent 会直接启用并运行，由目标 Hub 管理；它不依赖同机安装 Hub。安装器会同时部署同包的 `simadmin-device-service` 作为按需 WiFi Calling Worker，但不会安装或启动本机 Device Service 单元。指定版本时增加 `--version 0.0.6`，版本号可以带或不带 `v`。
...
升级前仍应在 Hub 中创建并下载备份。固定版本部署可把 `latest` 替换为发布标签，例如 `v0.0.6`。
