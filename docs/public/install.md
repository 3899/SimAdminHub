...
Hub 与 SimAdmin Agent 使用明确的协议版本，不兼容时会拒绝 WebSocket 会话，不能只根据页面能否打开判断兼容。优先使用同一发布周期明确支持 SimAdminHub 的 SimAdmin 版本；正式归档内的 `BUILD-INFO` 会记录构建时使用的 SimAdmin `main` 提交。
...
单独安装的 Host Agent 会直接启用并运行，由目标 Hub 管理；它不依赖同机安装 Hub。指定版本时增加 `--version 0.0.1`，版本号可以带或不带 `v`。
...
标准 Docker 安装会在首次启动时映射 `3001:3001`，并直接授予 `/dev`、只读 `/sys` 和 privileged 硬件访问权限。这样即使当前关闭 Host Agent，后续也能在系统设置中一键启用，无需重新创建或修改容器。关闭 Host Agent 只代表不运行硬件控制进程，不会撤销容器已经获得的权限。

需要修改外部访问端口时，只改端口映射左侧，例如把 Compose 中的 `"3001:3001"` 改成 `"8080:3001"`，或在 `docker run` 命令中使用 `-p 8080:3001`，随后通过宿主机的 `IP:8080` 访问。Hub 地址由当前访问地址自动推导，不需要填写或保存宿主机 IP。

标准 bridge 网络不保证局域网 mDNS 广播能够进入容器；自动发现不可用时，直接填写 SimAdmin 设备地址接入。为了让设备获得正确的回连地址，接入设备时应通过宿主机局域网 IP 打开 Hub，而不是使用浏览器中的 `127.0.0.1`。Docker Host Agent 默认使用 Direct AT、QMI 或 MBIM；如需复用宿主机 ModemManager，还必须按宿主机发行版额外映射系统 D-Bus。
...
升级前仍应在 Hub 中创建并下载备份。固定版本部署可把 `latest` 替换为发布标签，例如 `v0.0.1`。
...
网络设备不需要安装第二个 Agent，Hub Agent 能力已经内置在 SimAdmin 中。
...
自动发现只适用于支持 mDNS 的局域网。确认 SimAdmin 已切换到集中管理模式；不同 VLAN、访客网络或禁用组播时，直接填写设备完整地址。
...
检查协议、端口、防火墙和路由。Hub 无法访问设备时，改为在 SimAdmin 中填写 Hub 地址主动连接。
