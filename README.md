...
当前版本为单用户控制台，不提供 Web 登录、用户、角色或租户。顶层集中 OTA 和日志中心尚未开放；SimAdmin 子设备完整面板仍保留设备本地 OTA。
...
- [安全部署](docs/public/security.md)：无登录版本的网络边界、反向代理、敏感数据和密钥保护要求。
...
镜像同时包含 Hub 和独立 Host Agent。Host Agent 默认不启动，需要管理 Docker 宿主机直连模组时可在“系统设置 > 概览”中一键启用。容器首次创建时已经获得硬件访问权限；端口映射左侧的 `3001` 可按需修改。安全边界、Compose 和升级说明见[安装与部署](docs/public/install.md)。
...
第一版没有 Web 登录。不要将 Hub `3001` 端口直接暴露到公网；远程访问必须通过可信 VPN，或使用带 TLS、身份认证和来源限制的反向代理。顶部“隐藏敏感信息”只影响页面显示，不能代替访问控制或数据库加密。
